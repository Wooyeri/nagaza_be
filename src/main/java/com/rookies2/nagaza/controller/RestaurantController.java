...
public class RestaurantController {
    ...
    @PostMapping("/like/{id}")
    public ResponseEntity<RestaurantDto> toggleMovieLike(@PathVariable Integer restaurantId, @RequestParam Integer userId) {
        RestaurantDto restaurantDto = restaurantService.toggleLike(restaurantId, userId);
        return ResponseEntity.ok(restaurantDto);
    }

    @GetMapping("/like/{id}")
    public ResponseEntity<Boolean> isMovieLiked(@PathVariable Integer restaurantId, @RequestParam Integer userId) {
